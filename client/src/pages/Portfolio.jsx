...
const Portfolio = (props) => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      Promise.resolve(
        axios.get(
          `http://localhost:3000/api/accounts/${authUser.user_id}`,
          config
        )
      ),
      Promise.resolve(
        axios.get(
          `http://localhost:3000/api/accounts/balances/${authUser.user_id}`,
          config
        )
      ),
    ])
  ...
  return (
    <section id='portfolio' className='page'>
      <h1>
        ...
        <span>
          {account && `$${Number(account.account.total_balance).toFixed(2)}`}
        </span>
      </h1>
      <div className='portfoliobarchart'>
      ...
      <div className='holdings'>
        ...
        <div className='donut'>
          {account && <PortfolioDonutChart account={account} />}
        </div>
      </div>
